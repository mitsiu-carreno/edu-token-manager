...
namespace students{
  ...
  Student* AskVerification(MYSQL *conn, char code_input[5]){
    ...
    MYSQL_ROW row;
    char query[80];
    sprintf(query, "SELECT id, code, full_name, identity_confirmation FROM students WHERE code = '%s'", code_input);
    ...
    char confirmation [sizeof(row[3])];
    strcpy(confirmation, row[3]);
    printw("\n\n");
  ...
  bool ConfirmQuestion(MYSQL *conn, int question_number){
    MYSQL_RES *res;
    MYSQL_ROW row;
    char query[69];
    sprintf(query, "SELECT txt_question FROM answers WHERE num_question = %d", question_number);
    res = mysqlConn::Query(conn, query);
    if(!res){
    ...
    row = mysql_fetch_row(res);
    printw("Pregunta seleccionada:\n%s\nDeseas continuar y gastar un token? [s/n]: ", row[0]);
    mysql_free_result(res);
  ...
  int AskQuestionNum(MYSQL *conn, Student *current_student){
    while(true){
      printw("Ingresa el número de respuesta que quieres [0 = Salir]: ");
      ...
      if(question_number >= 1 && question_number <= 26){
        if(ConfirmQuestion(conn, question_number)){
